...
# Since you will save this in 'C:\Users\User1\maverickpaint25',
# this will automatically be the project root.
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))

# Set the name for the output file
OUTPUT_FILENAME = "maverickpaint25_summary.txt" 
...
def generate_file_tree(root_dir):
    ...
    # Uses the folder name as the project name in the tree header
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        # Calculate the current level in the tree for indentation
        # Use os.path.relpath for robust calculation
        relative_path_segment = os.path.relpath(root, root_dir)
        level = relative_path_segment.count(os.sep) if relative_path_segment != "." else 0
        indent = '│   ' * level

        # Add the directory to the tree, skipping the root folder itself
        if root != root_dir:
